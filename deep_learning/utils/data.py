...
class InriaDataset(Dataset):
    ...
    Random access Dataset for datasets of tiffs stored like this:
        ...
        ...
    """
    def __init__(self, root, parts=['images', 'gt'], transform=None, suffix='.jpg'):
        self.root = Path(root)
        self.parts = parts
            ...
            if not cache_dir.is_dir() or lockfile.exists():
                ...
                for file in files:
                    with rio.open(file) as raster:
                        ...
                        # Map values to binary format: 0, 1
    ...
    def __getitem__(self, idx):
        ...
        data = [torch.from_numpy(data_dict[p].astype(np.float32)) for p in self.parts]
