...
def extract_edge_coordinates(edge_pred, threshold=0.5):
    # Threshold the predicted edges
    edge_binary = (edge_pred > threshold).cpu().numpy().astype(np.uint8)
    ...
    for contour in contours:
        coordinates = contour.squeeze(1).tolist()
        edge_coordinates.append(coordinates)
