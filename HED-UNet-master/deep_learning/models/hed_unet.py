...
class HEDUNet(nn.Module):
    ...
    def __init__(self, input_channels=4, output_channels=2, base_channels=16,
    ...
    def forward(self, x):
        ...
        for block in self.down_blocks:
            ...
            x = block(x)
            print(f"Down Block Output Size: {x.size()}")

        multilevel_features = [x]
        for block, skip in zip(self.up_blocks, reversed(skip_connections)):
            x = block(x, skip)
            multilevel_features.append(x)
            print(f"Up Block Output Size: {x.size()}")
            print(f"Skip Connection Size: {skip.size()}")  # Add this line
        ...
        print(f"Concatenated Predictions Size: {predictions.size()}")
